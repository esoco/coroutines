...
public class Continuation<T> extends RelatedObject implements Executor {
    ...
    public final <V> void continueAccept(
        ...
        if (!bCancelled) {
            rCurrentExecution = rPreviousExecution.thenAcceptAsync(fNext, this)
    ...
    public final <I, O> void continueApply(
        CompletableFuture<I> rPreviousExecution, Function<I, O> fNext,
    ...
    public String toString() {
        return String.format("%s-%d[%s]", getCurrentCoroutine().get(NAME), nId,
            rResult);
    }

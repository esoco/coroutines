...
public class Continuation<T> extends RelatedObject implements Executor {
	...
	public <O> O fail(Throwable error) {
		if (!finished) {
			...
			getConfiguration(EXCEPTION_HANDLER, null).accept(error);
	...
	public <V> V getConfiguration(RelationType<V> configType, V defaultValue) {
		V rValue = defaultValue;

		if (hasRelation(configType)) {
			rValue = get(configType);
		} else if (scope.hasRelation(configType)) {
			rValue = scope.get(configType);
		} else if (scope.context().hasRelation(configType)) {
			rValue = scope.context().get(configType);
		} else {
	...
	public <V> V getState(RelationType<V> stateType, V defaultValue) {
		...
		} else if (hasRelation(stateType)) {
			value = get(stateType);
		} else if (scope.hasRelation(stateType)) {
			value = scope.get(stateType);
		}
		return value;
	}
	...
	public String toString() {
		return String.format("%s-%d[%s]", getCurrentCoroutine().get(NAME), id,
			result);
	}
	...
	void subroutineFinished() {
		closeManagedResources(getCurrentCoroutine(),
			getConfiguration(EXCEPTION_HANDLER, null));
